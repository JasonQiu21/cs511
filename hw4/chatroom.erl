...
do_propegate_message(State, Ref, ClientPID, Message) ->
	...
	lists:foreach(fun(El) -> propogate_message(State, Ref, ClientPID, Message, El) end, maps:keys(State#chat_st.registrations)),
	{ok, ClientNick} = maps:find(ClientPID, State#chat_st.registrations),
	State#chat_st{
		history = lists:append(State#chat_st.history, [{ClientNick, Message}])
	}.
