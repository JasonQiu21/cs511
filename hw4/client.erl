...
do_join(State, Ref, ChatName) ->
	case maps:find(ChatName, State#cl_st.con_ch) of
		...
		error -> 
			...
			receive
				{ChatPid, Ref_, connect, History} ->
					whereis(list_to_atom(State#cl_st.gui))!{result, self(), Ref_, History},
					{{dummy_target, dummy_response}, State#cl_st{
