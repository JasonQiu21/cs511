...
do_join(ChatName, ClientPID, Ref, State) ->
	...
	case maps:find(ChatName, State#serv_st.chatrooms) of
		{ok, ChatPid} ->
			...
			State#serv_st{registrations = maps:put(ChatName, lists:append(Regs, [ClientPID]), State#serv_st.registrations)};
		error ->
...
do_leave(ChatName, ClientPID, Ref, State) ->
	...
	State#serv_st{
		registrations = maps:put(ChatName, lists:delete(ClientPID, Regs), State#serv_st.registrations)}.
	...
	case lists:member(NewNick, maps:values(State#serv_st.nicks)) of 
		...
		false -> 
			...
			case maps:find(ClientPID, State#serv_st.registrations) of
				{ok, Chatrooms} -> 
					lists:foreach(fun(El) -> io:format("chat ~p~n", El), El!{self(), Ref, update_nick, ClientPID, NewNick} end, Chatrooms);
				error -> ok
			end,
			State#serv_st{
...
do_client_quit(State, Ref, ClientPID) ->
	case maps:find(ClientPID, State#serv_st.registrations) of
		{ok, Chatrooms} -> lists:foreach(fun(El) -> El!{self(), Ref, unregister, ClientPID} end,Chatrooms);
		error -> ok
	end, 
